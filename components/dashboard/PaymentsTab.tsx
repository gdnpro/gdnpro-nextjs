                      ...
                      <div className="flex items-center">
                        ...
                        (userType === "freelancer" && (transaction as any).client?.rating) ? (
                          <>
                            ...
                            <span>
                              {userType === "client"
                                ? (transaction as any).freelancer?.rating?.toFixed(1)
                                : (transaction as any).client?.rating?.toFixed(1)}
                            </span>
                          </>
                        ) : null}
                      </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 pb-24 sm:p-8">
              <div className="space-y-6">
                ...
                {((userType === "client" && (selectedTransaction as any).freelancer) ||
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-6 shadow-sm">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-100 to-teal-100 ring-2 ring-cyan-200">
                        {userType === "client" &&
                        (selectedTransaction as any).freelancer?.avatar_url ? (
                          <img
                            src={(selectedTransaction as any).freelancer.avatar_url}
                            alt={(selectedTransaction as any).freelancer.full_name}
                            className="h-full w-full object-cover"
                          />
                        ) : userType === "freelancer" &&
                          (selectedTransaction as any).client?.avatar_url ? (
                          <img
                            src={(selectedTransaction as any).client.avatar_url}
                            alt={(selectedTransaction as any).client.full_name}
                            className="h-full w-full object-cover"
                          />
                        ) : (
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900">
                          {userType === "client"
                            ? (selectedTransaction as any).freelancer?.full_name
                            : (selectedTransaction as any).client?.full_name}
                        </h4>
                        <p className="text-sm text-gray-600">
                          {userType === "client"
                            ? (selectedTransaction as any).freelancer?.email
                            : (selectedTransaction as any).client?.email}
                        </p>
                        {(userType === "client" && (selectedTransaction as any).freelancer?.rating) ||
                        (userType === "freelancer" && (selectedTransaction as any).client?.rating) ? (
                          <div className="mt-1 flex items-center gap-1 text-sm text-cyan-600">
                            <i className="ri-star-fill text-yellow-300"></i>
                            <span className="font-medium">
                              {userType === "client"
                                ? (selectedTransaction as any).freelancer?.rating?.toFixed(1)
                                : (selectedTransaction as any).client?.rating?.toFixed(1)}
                            </span>
                          </div>
                        ) : null}
                      </div>
