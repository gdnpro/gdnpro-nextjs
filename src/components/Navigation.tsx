...
export default function Navigation() {
  ...
  const goToDashboard = () => {
    if (user?.role === "admin") navigate("/admin/contacts")
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm lg:px-8 sm:px-6 px-4">
      <div className="max-w-7xl px-6 mx-auto">
        <div className="flex gap-8 justify-between items-center h-16">
          ...
          <button
            onClick={goToHome}
            className={`${user ? "hidden md:block" : "block"}`}
          >
          ...
          {!loading && user ? (
            <div className="relative hidden md:block">
              ...
              {/* Dropdown */}
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-3 w-48 bg-white rounded-lg shadow-lg border py-2 z-50">
                  <button
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button
                ...
                className="bg-primary hover:bg-cyan-700 text-white px-6 py-2 rounded-full font-semibold transition-all transform hover:scale-105"
              >
                Registrarse
              </button>
            </div>
